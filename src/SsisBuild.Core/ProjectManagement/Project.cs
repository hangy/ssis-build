...
public sealed class Project : IProject
{
    ...
    public void Save(Stream destinationStream, ProtectionLevel protectionLevel, string password)
    {
        ...
        {
            var name = PackUriHelper.CreatePartUri(new Uri(package.Key, UriKind.Relative)).OriginalString[1..];
        ...
        {
            var name = PackUriHelper.CreatePartUri(new Uri(projectConnection.Key, UriKind.Relative)).OriginalString[1..];
